...
class CountQuote:
    ...
    def recode(self):
        """Recode labels."""
        label1 = pd.read_csv(self.path + 'quoteLabel_all.csv')
        quote_or_lyrics = pd.read_csv(self.path + 'quote_all_labels.csv')
        quote_or_lyrics = quote_or_lyrics[['textID','tag']]
        labels = label1.merge(quote_or_lyrics)
        labels['label'] = labels['label'].replace(['NonQuote','Quote','quote'], [0, 1, 1])
        labels['tag'] = labels['tag'].replace(['NotQuote','lyrics','quote'], [0, 2, 1])
        return labels[['textID','label', 'tag']]
